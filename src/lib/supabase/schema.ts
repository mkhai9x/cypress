...
export const workpaces = pgTable('workspaces', {
  id: uuid('id').defaultRandom().primaryKey().notNull(),
  createdAt: timestamp('created_at', {
    withTimezone: true,
    mode: 'string',
  }),
  workpaceOwner: uuid('workspace_owner').notNull(),
...
export const folders = pgTable('folders', {
  ...
  inTrash: text('in_trash'),
  bannerUrl: text('banner_url'),
  workpaceId: uuid('workspace_id').references(() => workpaces.id, { onDelete: 'cascade' }),
});
